...
-- double precision in postgres x exist, so ::numeric
select item_purchased, 
...
-- customer segmentation and to prove customer loyalty , cte
...
-- window fuction- rank, dense rank, row number--> for many items, the numbers are same
with item_counts as( 
select category, item_purchased, 
count(customer_id) as total_orders, 
row_number() over (partition by categoryorder by count(customer_id) desc)as item_rank
from customer 
